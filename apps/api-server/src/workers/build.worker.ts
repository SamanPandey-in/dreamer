 ...
 * THEN call ECS, THEN write the ARN — with no way to know whether a
...
export function startBuildWorker() {
  const worker = new Worker<BuildJob>(
    ...
    (job: Job<BuildJob>) =>
      runWithContext({ correlationId: job.data.deploymentId, source: 'build-worker' }, async () => {
        ...
        if (withArn.cancelRequested) {
          // A Stop landed after this worker won the QUEUED->LAUNCHING claim
          // but before an ARN existed to act on — stopDeployment could only
          // flag intent (see its cancelRequested branch). This is that flag
          // being honored, the moment there's actually a task to stop.
          //
          ...
          // seconds-long "AWS RunTaskCommand in flight" window the atomic
          // claim above closes — accepted as an acceptable residual rather
          // than adding a second lock for a gap this narrow.
          logger.info('Cancellation was requested while launching — stopping the task that just started', {
            ecsTaskArn: handle.ecsTaskArn,
          });
          try {
            await deploymentEngine.stopBuildTask(handle.ecsTaskArn);
          } catch (err) {
            logger.error('Failed to stop late-launched task after cancellation', { err });
          }
  ...
  worker.on('ready', () => {
    ...
    // note in docs/SELF-HOSTING.md. A queued deployment that never moves
    // past QUEUED and never shows an ECS task in the AWS console almost
