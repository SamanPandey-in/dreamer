...
const FRAMEWORK_TEMPLATES = {
    NEXT_SSR: 'nextjs-lambda.dockerfile',
}
    ...
    for (const filename of candidates) {
        ...
        if (!/output\s*:\s*['"]standalone['"]/.test(contents)) {
            publishLog(
                ...
                `The Lambda image build will likely fail at the ".next/standalone" copy step — ` +
...
/**
 * Returns the ABSOLUTE PATH to the Dockerfile Kaniko should build, and as
 * a side effect, writes a generated one into buildContextPath if the repo
 * doesn't already ship its own — same "config wins over convention"
 * precedent as Project.buildCommand/outputDirectory overriding
 * script.js's own defaults elsewhere in this file.
 */
