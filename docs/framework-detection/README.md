...
The root-directory picker calls this **once per folder you actually
expand** — not a recursive walk of the entire tree up front. A repo with
thousands of files costs the picker exactly as many API calls as the
number of folders you actually clicked into, not one call per file in
...
All of these calls authenticate with the same stored Personal Access
Token used everywhere else (see [`auth/README.md`](../auth/README.md)) —
public repos work without one; private repos are detected through the
same token that will later clone them.
...
- **`angular`'s `defaultOutputDirectory` is a deliberate guess, not a
  ...
  you to confirm or correct in the wizard.

Every preset defaults `requiresUnsupportedRuntime: false` except one:
**`nuxt` is still flagged `requiresUnsupportedRuntime: true`** — Nuxt/Vue's
dynamic runtime has no Dockerfile template yet, so the wizard surfaces a
blocking notice instead of silently producing a deployment that builds
successfully and then 404s on every route.
...
exists, Vite is genuinely building this project, full stop, regardless of
what happens to be listed (or not listed, in an unusual monorepo
...
detection legible (and, later, useful for improving detection quality):
"Detected via `next.config.js`" reads very differently than an
unexplained "Detected: Next.js" would, and knowing whether real-world
repos get caught by the config-file check or fall through to the
dependency check is exactly the kind of signal worth keeping even before
there's a UI surface consuming it.
...
you pick a different preset from the dropdown
...
wait on a fresh GitHub API round trip just because you decided you
...
gets written to `Project` once you submit, and why it's a snapshot
