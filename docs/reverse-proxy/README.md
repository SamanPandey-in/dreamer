...
[Self-Hosting Guide](../SELF-HOSTING.md), where it's just another
...
**STATIC** — proxies to an S3 HTTPS URL built from the subdomain
...
**DYNAMIC** — proxies straight to the deployment's own Lambda Function
URL, with `changeOrigin: true` (the opposite of what you'd want for a
shared, host-header-routed front door — see
[dynamic deployments](../deployments/dynamic-deployments.md#serving-how-reverse-proxy-reaches-a-lambda-function)
for exactly why). No S3-specific rewrite applies here at all — Lambda's
own HTTP-facing layer (via the Lambda Web Adapter inside the function)
handles routing the request to the app's own server unmodified.

## Self-hosted topology: nginx in front, `reverse-proxy` behind it

When self-hosting the whole platform (see the
[Self-Hosting Guide](../SELF-HOSTING.md)), `reverse-proxy` itself never
